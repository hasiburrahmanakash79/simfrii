...
const Payment = () => {
  ...
  const services = [
    { id: "ORD-AX93K7-1", planName: "Asia 7 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Phoenix Baker", service_type: "Regular", price: "$20", status: "Active" },
    { id: "ORD-AX93K7-2", planName: "Asia 365 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Drew Cano", service_type: "Regular", price: "$35", status: "Canceled" },
    { id: "ORD-AX93K7-3", planName: "Asia 1 year", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Phoenix Baker", service_type: "Regular", price: "$18", status: "Active" },
    { id: "ORD-AX93K7-4", planName: "Asia 7 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Drew Cano", service_type: "Regular", price: "$20", status: "Pending" },
    { id: "ORD-AX93K7-5", planName: "Asia 7 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Phoenix Baker", service_type: "Regular", price: "$35", status: "Active" },
    { id: "ORD-AX93K7-6", planName: "Asia 7 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Drew Cano", service_type: "Regular", price: "$18", status: "Canceled" },
    { id: "ORD-AX93K7-7", planName: "Asia 7 days", date: "Jan 6, 2025", PaymentMethod: "VISA", customerName: "Phoenix Baker", service_type: "Custom", price: "$20", status: "Active" },
  ];

  // Dynamic stats based on services
  const totalRevenue = services.reduce((sum, service) => sum + parseFloat(service.price.replace("$", "")), 0);
  const activeServices = services.filter((service) => service.status === "Active").length;
  const canceledServices = services.filter((service) => service.status === "Canceled").length;
  const stats = [
    { title: "Total Revenue", value: `$${totalRevenue.toFixed(0)}`, change: "40%", changeType: "increase", comparison: "vs last month" },
    { title: "Active Earning", value: activeServices, change: "20%", changeType: "decrease", comparison: "vs last month" },
    { title: "Total Cancel", value: canceledServices, change: "20%", changeType: "decrease", comparison: "From last month" },
  ];

  const filteredServices = services.filter((service) =>
    [service.planName, service.customerName, service.PaymentMethod, service.service_type].some((field) =>
  ...
  return (
    <div className="">
      <SectionTitle title="Payment" description="Track payment of your customers." />
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6">
          <div className="flex justify-between items-center">
            <div>
                <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Payment</h1>
            ...
            <tbody>
              {currentServices.length > 0 ? currentServices.map((service) => (
                <tr key={service.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="p-4 text-gray-700"><Link to={`/admin/payment-details/${service.id}`}>{service.planName}</Link></td>
                  <td className="p-4 text-gray-700">{service.customerName}</td>
                  <td className="p-4 text-gray-700">{service.date}</td>
                  <td className="p-4 text-gray-700">{service.price}</td>
                  <td className="p-4 text-gray-700">{service.PaymentMethod}</td>
                  <td className="p-4">
                ...
                <tr>
                  <td colSpan="6" className="text-center py-6 text-gray-500 font-medium">No payments found matching your criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
